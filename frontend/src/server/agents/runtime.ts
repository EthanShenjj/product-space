...
export type AgentBuilder = (model: Model, pageContext?: string) => Agent<any, any>;

export async function runWithFallback(
  buildAgent: AgentBuilder,
  input: string | AgentInputItem[],
  options: { customModel?: CustomModelConfig; session?: Session; stream?: false; workflowName: string; groupId?: string; pageContext?: string },
) {
  ...
  for (const provider of attempts) {
    try {
      const result = await run(buildAgent(getAgentModel(provider), options.pageContext), input, {
...
export async function streamWithFallback(
  buildAgent: AgentBuilder,
  input: string | AgentInputItem[],
  options: { customModel?: CustomModelConfig; session?: Session; workflowName: string; groupId?: string; pageContext?: string },
